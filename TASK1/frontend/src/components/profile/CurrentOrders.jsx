...
const CurrentOrders = ({ orders, onCancelOrder }) => {
  ...
  if (orders.length === 0) {
    return <p className="text-muted-foreground">No current orders.</p>;
  }
              ...
              <tr key={order._id} className="hover:bg-muted/50">
                ...
                <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                  ...
                  {order.status === 'Pending' && (
                    <button onClick={() => onCancelOrder(order._id)} className="text-destructive hover:text-destructive/80">Cancel Order</button>
                  )}
                </td>
              </tr>
            ))}
