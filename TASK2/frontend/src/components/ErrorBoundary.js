  ...
  render() {
    if (this.state.hasError) {
      if (this.props.fallback) {
        return this.props.fallback(this.state.error, this.state.errorMessage);
      }
        ...
        <div className="error-boundary min-h-screen flex items-center justify-center bg-gray-50">
          <div className="max-w-md w-full bg-white shadow-lg rounded-lg p-6 text-center">
            ...
            <div className="space-y-2">
              ...
              <button
                onClick={() => this.setState({ hasError: false, error: null, errorMessage: '' })}
            ...
            {process.env.NODE_ENV === 'development' && (
              <details className="mt-4 text-left">
                ...
                <pre className="mt-2 text-xs text-red-600 bg-red-50 p-2 rounded overflow-auto">
                  {this.state.error?.stack || this.state.errorMessage}
                </pre>
              </details>
            )}
          </div>
